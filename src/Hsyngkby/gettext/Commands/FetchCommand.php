<?php
namespace Hsyngkby\gettext\Commands;
...
class FetchCommand extends Command {
    ...
    public function fire ()
    {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN')
            throw new \Hsyngkby\gettext\FetchCommandNotSupportedOnWindowsException("The fetch command requires the cli command 'locale' to be available; this is not available on a windows system");
        ...
        if (!File::isDirectory($config_path) || !File::isFile($locales_file) || !File::isFile($encodings_file) || !File::isFile($locales_dist_file) || !File::isFile($encodings_dist_file))
        {
            ...
            $this->call("config:publish", array("package" => "hsyngkby/gettext"));
        }
        else
        ...
        if (!File::isWritable($locales_file) || !File::isWritable($encodings_file))
            throw new \Hsyngkby\gettext\ConfigFilesNotWritableException("the package config files are not writable; please check your file permissions and try again");
        else
        ...
        if (!$process->isSuccessful())
            throw new \Hsyngkby\gettext\CannotFetchInstalledLocalesException("Could not execute the locale command to retrieve installed locales on this system");
        ...
        foreach ($list as $item)
        {
            ...
            if ($count != 1 && $count != 2)
                throw new \Hsyngkby\gettext\InvalidItemCountException("The item [$item] contains more than 2 sections (separated by a dot)" . PHP_EOL . "I could not determine the locale and encoding of this item automatically");
